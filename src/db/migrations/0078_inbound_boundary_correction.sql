...
--      to exactly one company (migration 0075), so the identity is company-scoped WITHOUT depending
...
comment on column public.source_events.provider_account_id is
  'OUR account that received the message (WhatsApp metadata.phone_number_id). Trusted: the provider sets it and a sender cannot influence it. Scopes the canonical identity and resolves the company (migration 0075).';
...
--
-- The correct call is the actor-scoped form migration 0076 introduced for exactly this purpose.
-- ─────────────────────────────────────────────────────────────────────────────────────────────
