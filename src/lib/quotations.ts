  ...
  for (const it of items ?? []) {
    ...
    if (match && normalizeCurrency(match.currency) === qCurrency && qCurrency !== "" && isAutoPriceableQuantity(it.quantity)) {
      ...
      if (!existing) {
        ...
        // `client`, not `db`: the candidate renamed the injected handle so this module stays
        // client-agnostic. Main's diagnostic above is kept verbatim; only the name changes.
