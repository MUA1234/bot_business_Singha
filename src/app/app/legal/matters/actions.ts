...
export async function createMatter(formData: FormData): Promise<void> {
  ...
  const { error } = await supabaseWriteClient().from("legal_matters").insert({ company_id: p.companyId, title, matter_type, status: "open", owner_id: p.userId });
  if (error) return;
...
export async function setMatterStatus(formData: FormData): Promise<void> {
  ...
  const { data: m } = await supabaseWriteClient().from("legal_matters").select("id").eq("id", id).eq("company_id", p.companyId).maybeSingle();
  if (!m) return;
  await supabaseWriteClient().from("legal_matters").update({ status }).eq("id", id).eq("company_id", p.companyId);
  revalidatePath("/app/legal/matters");
}
