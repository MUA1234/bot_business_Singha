...
export async function recordCashCount(formData: FormData): Promise<void> {
  ...
  const counted = Number(formData.get("counted_amount") ?? NaN);
  if (!cashAccountId || !Number.isFinite(counted) || counted < 0) return;
  ...
  await db.from("cash_counts").insert({ company_id: p.companyId, cash_account_id: cashAccountId, counted_amount: counted, variance: Number(variance), counted_by: p.userId });
