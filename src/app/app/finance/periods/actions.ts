...
export async function setPeriodStatus(formData: FormData): Promise<void> {
  ...
  const { data: period } = await supabaseWriteClient().from("accounting_periods").select("id").eq("id", id).eq("company_id", p.companyId).maybeSingle();
  if (!period) return;
  ...
  await supabaseWriteClient().from("accounting_periods").update(patch).eq("id", id).eq("company_id", p.companyId);
