...
function SubmitButton() {
  ...
  return (
    <Button type="submit" loading={pending} aria-label="Analyse update">
  ...
  return (
    <div className="stack gap-3">
      <div className="grid cols-3">
        <Card className="stat">
          <div className="k">Tasks captured</div>
          <div className="v" style={{ fontSize: "1.6rem", color: "var(--ok)" }}>{r.createdTasks}</div>
        </Card>
        <Card className="stat">
          <div className="k">Confidence</div>
          <div className="v" style={{ fontSize: "1.6rem" }}>{Math.round(r.confidence * 100)}%</div>
        </Card>
        <Card className="stat">
          <div className="k">Authority</div>
          <div className="v" style={{ fontSize: "1.1rem" }}>{r.requiredAuthority.replace(/_/g, " ")}</div>
        </Card>
      </div>
      ...
      {r.confirmedFacts.length > 0 && (
        <Card>
          <CardHeader title="Confirmed facts" />
          <CardBody padding="sm">
      ...
      {r.suggestedActions.length > 0 && (
        <Card>
          <CardHeader title={<>Suggested actions <span className="dim small">(for you to decide — not auto-run)</span></>} />
          <CardBody padding="sm">
