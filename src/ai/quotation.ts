...
export async function runQuotationTurn(
  ...
  try {
    resp = await transport.complete({ model, system: SYSTEM_PROMPT, user, maxTokens, jsonSchema: QUOTATION_TURN_JSON_SCHEMA });
    text = resp.text;
...
/**
 * Policy-routed quotation turn: the same customer-facing turn, but selected and budgeted by
 * the model-policy router rather than calling one fixed transport.
 *
 * Kept alongside `runQuotationTurn` rather than replacing it: the direct form is still the
 * one the unit tests drive with a stub transport, and it is the fallback when no policy
 * budget is configured for the company.
 *
 * Merge note (Release 1): this arrives from the recovery line, and now also applies
 * `normalizeTurnShape` before validation — the tolerance `main` added after a live model
 * returned flat customer fields and `item` instead of `description`, silently discarding the
 * collected name. Both entry points must survive the same model drift, or fixing one leaves
 * the other broken.
 */
export async function runPolicyRoutedQuotationTurn(
  ...
  const raw = safeJson(execution.response.text);
  const parsed = QuotationTurnSchema.safeParse(normalizeTurnShape(raw));
  const run: AiRunRecord = {
    ai_run_id: `ai_${randomUUID()}`,
    route: "quotation",
    ...
    validation_issues: parsed.success
      ? undefined
      : raw === null
        ? [`no_json_output: ${execution.response.text.length} chars`]
  ...
  await ledger.record(run);
  if (!parsed.success) {
    // Distinguish "the model emitted nothing parseable" from "the shape was wrong", the same
    // way the direct path does — `validation_failed` hid the first case behind the second.
    const reason = raw === null
      ? `no_json_output: ${execution.response.text.length} chars`
      : `validation_failed: ${parsed.error.message}`;
    return { ok: false, reason };
  }
  return { ok: true, turn: parsed.data };
}
