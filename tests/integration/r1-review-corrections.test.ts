...
describe.skipIf(!enabled)("R1 correction loop 1 (disposable local PostgreSQL)", () => {
  ...
  it("OF-014 (the DISCLOSURE half): migration 0085 stops a forged claim disclosing a quotation's status", async () => {
    // Renamed after security review 2. The old name said "OF-014: CLOSED by migration 0085",
    ...
    // finance RPCs until migration 0087 (G-01). This test only ever covered the disclosure half.
    const q = (await row(
