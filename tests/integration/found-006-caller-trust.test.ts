...
describe.skipIf(!enabled)("FOUND-006 — privilege decides, request text does not", () => {
   ...
   *
   * This is NOT closed by migration 0085 and cannot be — the mitigation is topological (the service
  ...
  it("only TWO api-reachable SECURITY DEFINER functions may consult caller_jwt_role at all", async () => {
    ...
    // else, in any syntax, fails here and fails migration 0086's own assertion.
  ...
  it("every api-reachable definer function that can REACH claim text is on the reviewed allowlist", async () => {
    ...
    // fails here until someone adds it deliberately. Mirrors migration 0087's assertion exactly.
    const found = (await db.query(
      `with recursive
         seed as (
    ...
    expect(found.sort()).toEqual([
      ...
      // OF-016 (migration 0088). Both reach claim text only to learn WHO the caller is —
  ...
  it("G-01: a forged service_role claim cannot post a material journal — genuine unprivileged role", async () => {
    ...
    // migration 0087 it posted a 999,999 journal with no capability and no human actor, because
