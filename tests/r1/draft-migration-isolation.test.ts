...
describe("released migrations are untouched by R1", () => {
  // Release 1: the sequence is 110, not 109, because `main`'s
  // 0069_company_routing_and_catalogue_department joined it and the recovery line shifted +1 to
  // 0070–0110. The assertion this test exists to make is UNCHANGED — the R1 draft units still
  // hold none of these numbers — and it is now checked directly rather than inferred from a
  // count, so a future renumbering cannot break the test without breaking the property.
  it("the production migration sequence is contiguous 0001–0110 and holds no R1 draft unit", () => {
    const prod = readdirSync(PROD_DIR).filter((f) => PROD_FILE_RE.test(f)).sort();
    expect(prod).toHaveLength(110);
    expect(prod[0]!.slice(0, 4)).toBe("0001");
    expect(prod[prod.length - 1]!.slice(0, 4)).toBe("0110");
    // Contiguous: a gap would mean a renumbering dropped or double-moved a file.
    prod.forEach((f, i) => expect(f.slice(0, 4)).toBe(String(i + 1).padStart(4, "0")));
    // No draft unit smuggled itself into the released sequence under a production name.
    expect(prod.filter((f) => /r1_draft/i.test(f))).toEqual([]);
  });
