...
<!-- Merge 2026-09-11: BOTH records are kept and neither is edited.
     Above: the candidate's Release 1 banner - the recovery line renumbered, CASE A proven,
     hosted high-water 0069.
     Below: main's record that 0070_identity_backfill_and_event_lifecycle.sql was APPLIED TO
     PRODUCTION over REST on 2026-09-11 while the ledger stayed at 0069.
     These are not alternatives. They are two true statements about the same database, and the
     integration that follows has to satisfy both. See
     docs/release-1/MAIN-0070-RECONCILIATION.md. -->
_Last reviewed: 2026-09-11 — **migrations 0048–0069 APPLIED TO PRODUCTION**; **0070 authored, verified
on a disposable PostgreSQL 16, NOT applied to any hosted environment (owner authorisation required)**._
...
> **2026-09-11 production application of 0070 (owner-authorised) — READ THIS BEFORE RUNNING `migrate`.**
> The repair in `0070_identity_backfill_and_event_lifecycle.sql` was applied to production, but
> **not by the migration runner**, and the `schema_migrations` ledger therefore still shows **0069 as
> the last applied version**. That is deliberate, not drift:
>
> - **Why.** `db.gazjughejdzebathpscb.supabase.co` publishes only an **AAAA** record and the operator
>   machine had **no global IPv6 address and no IPv6 route** (`ifconfig`: zero global v6; direct
>   connect: `No route to host`), so `psql`, `pg_dump` and `npm run migrate` could not reach it.
>   Every Supavisor pooler region answered `(ENOTFOUND) tenant/user postgres.gazjughejdzebathpscb
>   not found`, so there was no IPv4 Postgres path either. **A full `pg_dump` was therefore NOT
>   possible**; a targeted REST snapshot of all seven affected tables was taken instead
>   (`pre-0070-backup-20260911T030401Z.json`, 29 KB: profiles 8, users 3, memberships 3,
>   membership_roles 5, source_events 14, management_cases 3, tasks 7).
> - **How.** `scripts/hosted-repair-0070.mjs` performed the identical row changes over the
>   service-role REST API — idempotent, evidence-only, and one-way on access — then verified the
>   same invariants `tests/integration/identity-consistency.test.ts` asserts.
> - **Result.** 5 `users` + 5 `memberships` + 5 role grants created (all **8/8** staff now hold a
>   complete identity: a users row, an active membership in their own company, and at least one
>   role; both admins hold `system_administrator`); **14/14** `source_events` moved
>   `received → processed` with the proving `handled_at` as `processed_at` and the company stamped;
>   **3** `management_cases` and **7** `tasks` un-authored (they recorded a companies.id as the
>   author). `audit_events` was deliberately left untouched — it is append-only by trigger.
> - **Consequence for the runner.** `npm run migrate:status` will report **1 pending (0070)**, which
>   is *accurate*: the SQL has not executed. Running it from a machine with connectivity is **safe
>   and expected** — every statement is idempotent, so on the repaired data it changes nothing, and
>   only then is the ledger row written. Do **not** hand-insert the ledger row.
...
| 0069 | company_routing_and_catalogue_department (`companies.whatsapp_phone_number_id` with a partial UNIQUE so two companies cannot claim one Meta number, `companies.default_price_confirmation_department`, `product_catalog.department`, both FK'd to `departments_catalog`) — replaces the compiled-in `DEFAULT_COMPANY_ID` and the constant `"sales"` routing | ✅ **applied to production** `gazjughejdzebathpscb` (owner-authorised 2026-09-01; ledger row `0069` at 12:10:52Z) |
| 0070 | identity_backfill_and_event_lifecycle — **data repair only, no schema change, idempotent.** (A) re-runs the 0010 profiles→`users`/`memberships`/`membership_roles` backfill, repairing employees created through the admin panel after 0010 (five live accounts had a profile but no identity row, so they were invisible to `has_membership()`/`has_capability()`/`lib/access.ts` and would be denied once `RLS_READS`/`RLS_WRITES` flip); suspends the membership of any deactivated profile (one-way — it never re-grants access). (B) closes out `source_events` left at `received` for ever (no code path ever advanced the lifecycle, so `/api/health` counted every delivered message as unprocessed) — ONLY where the matching inbound `wa_messages` row proves `handled_at`, stamping that evidence timestamp and the company from the same row. (C) nulls `management_cases.created_by` / `tasks.created_by` where the value is a **companies.id** — the unattended `ai-monitor` sweep passed the company id as the actor and neither column has an FK, so it was accepted silently; only rows whose author IS a company are touched, so a human actor cannot be erased. `audit_events` rows are deliberately NOT rewritten (append-only by trigger) | ✅ **effect APPLIED to production 2026-09-11 (owner-authorised), ledger row deliberately NOT written** — see the 2026-09-11 note at the top. Verified beforehand on a disposable PostgreSQL 16 — fresh `0001→0070` clean; upgrade rehearsal staged at `0069` with the exact live defect state (8 profiles / 3 users / 3 memberships, a handled + an unhandled source event, a company-authored case and task, plus a human-authored task) → 8 users / 8 memberships / correct roles, the deactivated profile suspended, ONLY the proven source event closed out, the company-authored rows un-authored and the human-authored task untouched; re-run is a no-op. Full suites green (unit 500/87 files; integration 327/42 files on 0001→0070). **NOT applied to any hosted environment — owner authorisation required.** |
