...
| ID | Sev | Finding |
|---|---|---|
| [PR-F-001](03-SHA-AND-DEPLOYMENT-COMPARISON.md#pr-f-001) | **P0** | **Migration number collision on `0069`.** `main` and the branch line each define a *different* migration numbered 0069. `scripts/migrate.mjs` keys `schema_migrations` on the 4-character prefix, so **on any database where version `0069` is already recorded**, the branch's `0070_durable_inbound_processing.sql` is silently **skipped**. **Corrected 2026-09-10:** the earlier wording said 0070–0109 "would then run against a schema missing its objects", and asserted this of "the production database". Measured on a disposable PG 16.10 (R0 integration prep, Rehearsal A): **0070–0075 apply and commit, then 0076 fails loudly** (`column "next_attempt_at" does not exist`), leaving a **partially migrated** database at ledger high-water `0075`. Whether production is in the precondition state is **UNKNOWN** — see [r0-integration/03-HOSTED-STATE-CHECKLIST.md](r0-integration/03-HOSTED-STATE-CHECKLIST.md). |
...
| [PR-F-014](03-SHA-AND-DEPLOYMENT-COMPARISON.md#pr-f-014) | **P1** | **No build provenance — ESCALATED 2026-09-02 (R0-F-007).** Owner inspection found the active Railway deployment was made by `railway up` from the **CLI**, with no Git SHA and no GitHub source shown. The deployed revision is **UNAVAILABLE, not merely unconfirmed**: the running artifact may correspond to no commit at all, cannot be reproduced or audited from git, and contradicts D-021's GitHub-deploy claim. The earlier inference from Railway's `main` auto-deploy is WITHDRAWN. |
