...
| **OF-010** | `task_duplicate_suggestions` has no producer, so semantic near-duplicates are never surfaced | P2 | AIM-002 | table exists, nothing writes it | Yes: no writer in `src/` | REPO | **Stays open.** Concern (2) of migration 0072; needs a similarity producer, which is its own requirement |
...
nobody — and a person cannot create one, because migration 0082 refuses `submitted_by_source =
...
Reviewed at `0fd5f7b` (migrations 0085 + 0085). Verdict **CHANGES REQUESTED**. All seven are fixed
...
After migration 0087, the identical call:
...
**OF-018 (P2, not exploitable).** `resolve_inbound_review` and `record_inbound_review` (migration 0076) guard with `caller_jwt_role() is distinct from 'service_role'` **in addition to** their
...
## OF-016 — RESOLVED by migration 0088 (pending independent review)
