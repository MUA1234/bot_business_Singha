...
|---|---|---|---|
| **OF-001** no scheduled drain | P0 reliability | REPO | **FIXED (§3)** — migration 0080 + `/api/cron/dispatch-drain` |
...
| **OF-004** single-tenant company bridge | P1 | OWNER-CFG | **SURFACE FIXED (§5)** — migration 0081 + `/app/admin/inbound-setup`. Values and activation stay an owner gate |
...
| **OF-007** service role can assert `actor_source='human'` | P0 security | REPO | **FIXED (§2)** — migration 0079. AIM-003 came **off** `locally_verified` before any code was written and is not restored on this package's own say-so |
...
| **OF-013** a system-submitted approval request could not be created | P0 functional | REPO | **FIXED (§7)** — migration 0082. Found by the extreme end-to-end run |
