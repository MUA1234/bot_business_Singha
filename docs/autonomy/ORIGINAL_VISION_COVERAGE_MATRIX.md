...
| AIM-002 | Task-level deduplication | P0 | **locally_verified** | src/management/ai-manager/analyze-conversation.ts and src/app/app/command/analyze/actions.ts — both production analysis paths now create tasks through create_task_deduplicated (migration 0074); tasks_set_identity_hash trigger fires on every task write | 707761c | hosted migration application |
...
| KRN-002 | Observation sources across every managed domain | P0 | **locally_verified** | src/app/api/management/cycle/route.ts -> src/kernel/cycle.ts (loads and runs all TWELVE registered sources) | c074548 | phase gate — not authorised before R4 |
